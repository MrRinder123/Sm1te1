...
- Implemented multi-user voice channels via WebRTC mesh signaling (participant presence, offer/answer/ICE relay, mute state propagation).
- Added Foxtro/FullFoxtro subscription system scaffold with plan APIs, subscribe/cancel endpoints, and server boost mechanics.
- Added Discord-like message pins and thread channels (create/list threads per channel).
- Added channel-level message search, pin management UI hooks, and moderation kick endpoint for server staff workflows.
