...
router.delete('/:serverId/members/:memberId', requireAuth, requireServerRole('MODERATOR'), async (req, res) => {
  const target = await prisma.serverMember.findUnique({ where: { id: req.params.memberId } });
  if (!target || target.serverId !== req.params.serverId) return res.status(404).json({ message: 'Member not found' });

  await prisma.serverMember.delete({ where: { id: req.params.memberId } });
  return res.status(204).send();
});

router.get('/:serverId/boosts', requireAuth, async (req, res) => {
  ...
  const level = boostCount >= 14 ? 3 : boostCount >= 7 ? 2 : boostCount >= 2 ? 1 : 0;

  return res.json({ boostCount, level, boosters: boosts.map((b) => b.user) });
});
